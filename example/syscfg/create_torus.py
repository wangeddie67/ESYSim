...
# File name : create_torus.py
# Function : Create network configuration file for torus topology, 
#            including ring/mesh/torus topology.
#
...
topology_dict = { "Ring":      syscfg.EsyNetCfg.NT_RING,
                  "2D-Mesh":   syscfg.EsyNetCfg.NT_MESH_2D,
                  "2D-Torus":  syscfg.EsyNetCfg.NT_TORUS_2D,
                  "Dia-Mesh":  syscfg.EsyNetCfg.NT_MESH_DIA,
                  "Dia-Torus": syscfg.EsyNetCfg.NT_TORUS_DIA }


def optionParser() :
    # argument parser
    parser = argparse.ArgumentParser(
        prog='create_torus.py', 
        description='Create Network Configuration File for Torus Topology' )
...
if __name__ == "__main__" :
    ...
    if args.verbose == 1 :
        # print configuration
        if topology in [ syscfg.EsyNetCfg.NT_MESH_2D, \
            syscfg.EsyNetCfg.NT_MESH_DIA ] :
            print( '  topology     = Mesh' )
        elif topology in [ syscfg.EsyNetCfg.NT_RING, \
            syscfg.EsyNetCfg.NT_TORUS_2D, \
            syscfg.EsyNetCfg.NT_TORUS_DIA ] :

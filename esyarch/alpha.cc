...
//初始化statis const成员变量
const TALPHA::RS_link TALPHA::RSLINK_NULL = {NULL,NULL,0};
const TALPHA::CV_link TALPHA::CVLINK_NULL = {NULL,0};
const TALPHA::md_reg_names_t TALPHA::md_reg_names[122]={
  /* name */	/* file */	/* reg */
...
const TALPHA::osf_flag_table_t TALPHA::osf_flag_table[OSF_FLAG_NUM] = {
...
TALPHA::TALPHA(CoreStruct* cs,int thread_id)
:TCore(1,cs,thread_id),sim_num_insn(0), sim_num_refs(0), sim_num_branches(0), sim_total_insn(0), sim_total_refs(0),
 ...
 //loader.cc
 ld_text_base(0),ld_text_size(0),ld_data_base(0),ld_data_size(0),ld_brk_point(0),ld_stack_base(0),ld_stack_size(0),
...
inline void TALPHA::exitrun(){
	//call when exit condition
	running = 0;
	Exitstates[actualid] = 1;
}

void TALPHA::reg_stat_init(Statistics & ss){
	int i;
...
void TALPHA::init_context(){
	...
	} else /* dl1 is defined */
	{
		...
		runcontext->cache_dl1 = cache_create(name, nsets, bsize, /* balloc */FALSE,
				...
				(& TALPHA::dl1_access_fn), /* hit lat */cache_dl1_lat);
		...
		if (cache_dl2_opt[0] == '\0')
			...
			runcontext->cache_dl2 = cache_create(name, nsets, bsize, /* balloc */FALSE,
					/* usize */0, assoc, cache_char2policy(c),
					(& TALPHA::dl2_access_fn), /* hit lat */cache_dl2_lat);
		}
	}
	...
	} else /* il1 is defined */
	{
		...
		runcontext->cache_il1 = cache_create(name, nsets, bsize, /* balloc */FALSE,
				/* usize */0, assoc, cache_char2policy(c),
				(& TALPHA::il1_access_fn), /* hit lat */cache_il1_lat);
		...
		if (cache_il2_opt[0] == '\0')
			...
			runcontext->cache_il2 = cache_create(name, nsets, bsize, /* balloc */FALSE,
					/* usize */0, assoc, cache_char2policy(c),
					(& TALPHA::il2_access_fn), /* hit lat */cache_il2_lat);
		}
	}
	/* use an I-TLB? */
	if (itlb_opt[0] == '\0')
		...
		runcontext->itlb = cache_create(name, nsets, bsize, /* balloc */FALSE,
				...
				cache_char2policy(c), (& TALPHA::itlb_access_fn),
				/* hit latency */1);
	}
	/* use a D-TLB? */
	if (dtlb_opt[0] == '\0')
		...
		runcontext->dtlb = cache_create(name, nsets, bsize, /* balloc */FALSE,
				/* usize */sizeof(md_addr_t), assoc,
				cache_char2policy(c), (& TALPHA::dtlb_access_fn),
				/* hit latency */1);
	}
	//initial regs
...
int TALPHA::read_thread_args(FILE* bncfile){
...
void TALPHA::sim_load_prog(CoreStruct* cs){
...
sim_rt_t TALPHA::run_cycle(){
	{
		...
		if (!ruu_fetch_issue_delay)
		  ...
		  thispower->gpp_data->core_para->ifu_block_num++;
		}
		...
		//TODO
		//调用MCPAT结束
		//printf("ALPHA - simulation_cycle = %ld & sim_cycle=%ld & numthreads=%d\n",simulation_cycle,sim_cycle,actualid);
		sim_cycle++;
		if (max_insts && sim_num_insn >= max_insts){
			return MAX_INST;
		}
		thispower->gpp_data->core_para->total_cycle_num++;
		allpower->gpp_data->core_para->total_cycle_num++;
	}
	return NO_Q;
}
...
#endif /* PRINT_SYMS */
int TALPHA::acmp(const struct sym_sym_t **sym1, const struct sym_sym_t **sym2)
{
	return (int)((*sym1)->addr - (*sym2)->addr);
}
int TALPHA::ncmp(struct TALPHA::sym_sym_t **sym1, struct TALPHA::sym_sym_t **sym2)
{
	return strcmp((*sym1)->name, (*sym2)->name);
}
void TALPHA::sym_loadsyms(char *fname,	/* file name containing symbols */
	     int load_locals)	/* load local symbols */
{
  ...
  /* sort by address */
  qsort(sym_syms, sym_nsyms, sizeof(struct sym_sym_t *), (int (*)(const void *, const void *))(& TALPHA::acmp));

  /* sort by name */
  qsort(sym_syms_by_name, sym_nsyms, sizeof(struct sym_sym_t *), (int (*)(const void *, const void *))(& TALPHA::ncmp));
  ...
  /* sort by address */
  qsort(sym_textsyms, sym_ntextsyms, sizeof(struct sym_sym_t *), (int (*)(const void *, const void *))(& TALPHA::acmp));

  /* sort by name */
  qsort(sym_textsyms_by_name, sym_ntextsyms,
	sizeof(struct sym_sym_t *), (int (*)(const void *, const void *))(& TALPHA::ncmp));
  ...
  /* sort by address */
  qsort(sym_datasyms, sym_ndatasyms, sizeof(struct sym_sym_t *), (int (*)(const void *, const void *))(& TALPHA::acmp));
...
void TALPHA::sym_dumpsym(struct sym_sym_t *sym,	/* symbol to display */
...
void TALPHA::sym_dumpsyms(FILE *fd)			/* output stream */
{
  int i;
...
void TALPHA::sym_dumpstate(FILE *fd)			/* output stream */
{
	int i;
	if (fd == NULL)
	fd = stderr;
...
struct TALPHA::sym_sym_t *			/* symbol found, or NULL */
TALPHA::sym_bind_addr(md_addr_t addr,		/* address of symbol to locate */
...
struct TALPHA::sym_sym_t * TALPHA::	sym_bind_name(char *name,			/* symbol name to locate */
	      int *pindex,			/* ptr to index result var */
	      enum sym_db_t db)			/* symbol database to search */
{
...
FILE * TALPHA::eio_create(char *fname)
{
  FILE *fd;
...
FILE * TALPHA::eio_open(char *fname)
{
  FILE *fd;
  struct exo_term_t *exo;
...
int TALPHA::eio_valid(char *fname)
{
  FILE *fd;
  char buf[512];
...
counter_t TALPHA::eio_write_chkpt(struct regs_t *regs,		/* regs to dump */
...
/* read check point of architected state from stream FD, returns
   EIO transaction count (an EIO file pointer) */
counter_t TALPHA::eio_read_chkpt(struct regs_t *regs,		/* regs to dump */
		struct mem_t *mem,		/* memory to dump */
...
void
TALPHA::eio_read_trace(FILE *eio_fd,			/* EIO stream file desc */
...
void
TALPHA::eio_fast_forward(FILE *eio_fd, counter_t icnt)
{
...
/*end of eio.cc*/
/*cache.cc*/
struct TALPHA::cache_t *	TALPHA::cache_create(char *name,		/* name of the cache */
...
enum TCore::cache_policy TALPHA::cache_char2policy(char c)		/* replacement policy as a char */
{
  switch (c) {
  case 'l': return LRU;
...
void TALPHA::cache_config(struct cache_t *cp,	/* cache instance */
...
/* print cache stats */
void TALPHA::cache_stats(struct cache_t *cp,		/* cache instance */
	    FILE *stream)		/* output stream */
{
...
unsigned int TALPHA::cache_access(struct cache_t *cp,	/* cache to access */
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  if (repl->status & CACHE_BLK_VALID)
	...
	*repl_addr = CACHE_MK_BADDR(cp, repl->tag, set);
      ...
      if (repl->status & CACHE_BLK_DIRTY)
	{
	  ...
	  cp->writebacks++;
	  lat += (this->*cp->TALPHA::cache_t::blk_access_fn)(Write,
  ...
  /* read data block */
  lat += (this->*cp->TALPHA::cache_t::blk_access_fn)(Read, CACHE_BADDR(cp, addr), cp->bsize,
			   repl, now+lat);
...
int TALPHA::cache_probe(struct cache_t *cp,		/* cache instance to probe */
...
unsigned int TALPHA::cache_flush(struct cache_t *cp,		/* cache instance to flush */
...
unsigned int TALPHA::cache_flush_addr(struct cache_t *cp,	/* cache instance to flush */
...
void TALPHA::unlink_htab_ent(struct cache_t *cp,		/* cache to update */
...
void TALPHA::link_htab_ent(struct cache_t *cp,		/* cache to update */
	      struct cache_set_t *set,		/* set containing bkt chain */
...
void TALPHA::update_way_list(struct cache_set_t *set,	/* set contained way chain */
		struct cache_blk_t *blk,	/* block to insert */
...
/*end of cache.cc*/
/*bpred.cc*/
struct TALPHA::bpred_t *	TALPHA::bpred_create(enum bpred_class classt,	/* type of predictor to create */
...
struct TALPHA::bpred_dir_t *	TALPHA::bpred_dir_create (
...
void TALPHA::bpred_config(struct bpred_t *pred,	/* branch predictor instance */
	     FILE *stream)		/* output stream */
{
  switch (pred->classt) {
  case BPredComb:
...
void TALPHA::bpred_stats(struct bpred_t *pred,	/* branch predictor instance */
	    FILE *stream)		/* output stream */
{
...
void TALPHA::bpred_reg_stats(struct bpred_t *pred,	/* branch predictor instance */
...
void TALPHA::bpred_after_priming(struct bpred_t *bpred)
{
  if (bpred == NULL)
    return;
...
char * TALPHA::bpred_dir_lookup(struct bpred_dir_t *pred_dir,	/* branch dir predictor inst */
...
TALPHA::md_addr_t TALPHA::bpred_lookup(struct bpred_t *pred,	/* branch predictor instance */
	     md_addr_t baddr,		/* branch address */
...
void TALPHA::bpred_recover(struct bpred_t *pred,	/* branch predictor instance */
	      md_addr_t baddr,		/* branch address */
	      int stack_recover_idx)	/* Non-speculative top-of-stack;
					 * used on mispredict recovery */
{
...
void TALPHA::bpred_update(struct bpred_t *pred,	/* branch predictor instance */
	     md_addr_t baddr,		/* branch address */
...
/*machine.cc*/
char *
TALPHA::md_reg_name(enum md_reg_type rt, int reg)
{
  int i;
...
void TALPHA::md_print_ireg(md_gpr_t regs, int reg, FILE *stream)
{
  fprintf(stream, "%4s: %12d/0x%08x",
	  md_reg_name(rt_gpr, reg), regs[reg], regs[reg]);
}

void TALPHA::md_print_iregs(md_gpr_t regs, FILE *stream)
{
  int i;
...
void TALPHA::md_print_fpreg(md_fpr_t regs, int reg, FILE *stream)
{
	myfprintf(stream, "%4s: %16ld/0x%012lx/%f",
	md_reg_name(rt_fpr, reg), regs.q[reg], regs.q[reg], regs.d[reg]);
}
void TALPHA::md_print_fpregs(md_fpr_t regs, FILE *stream)
{
  int i;
...
void TALPHA::md_print_creg(md_ctrl_t regs, int reg, FILE *stream)
{
...
void TALPHA::md_print_cregs(md_ctrl_t regs, FILE *stream)
{
...
word_t TALPHA::md_xor_regs(struct regs_t *regs)
{
  int i;
  qword_t checksum = 0;
...
void TALPHA::md_init_decoder(void)
{
...
void TALPHA::md_print_insn(md_inst_t inst,		/* instruction to disassemble */
...
void TALPHA::regs_init(void)		/* register file to initialize */
{
...
/*regs.cc end*/
/*range.cc*/
char * TALPHA::range_parse_pos(char *pstr,			/* execution position string */
...
void TALPHA::range_print_pos(struct range_pos_t *pos,	/* execution position */
		FILE *stream)			/* output stream */
{
  switch (pos->ptype)
    {
...
char * TALPHA::range_parse_range(char *rstr,			/* execution range string */
...
void TALPHA::range_print_range(struct range_range_t *range,	/* execution range */
		  FILE *stream)			/* output stream */
{
...
int TALPHA::range_cmp_pos(struct range_pos_t *pos,		/* execution position */
...
int TALPHA::range_cmp_range(struct range_range_t *range,	/* execution range */
		counter_t val)			/* position value */
{
...
int TALPHA::range_cmp_range1(struct TCore::range_range_t *range,	/* execution range */
...
/*range.cc end*/
/*some of ptrace.cc/h*/
void TALPHA::ptrace_open(char *fname,		/* output filename */
...
void TALPHA::ptrace_close(void)
{
  if (ptrace_outfd != NULL && ptrace_outfd != stderr && ptrace_outfd != stdout)
    fclose(ptrace_outfd);
}
void TALPHA::__ptrace_endinst(unsigned int iseq)	/* instruction sequence number */
{
...
/* declare a new cycle */
void TALPHA::__ptrace_newcycle(tick_t cycle)		/* new cycle */
{
...
void TALPHA::__ptrace_newstage(unsigned int iseq,	/* instruction sequence number */
...
void TALPHA::__ptrace_newinst(unsigned int iseq,	/* instruction sequence number */
...
/* declare a new uop */
void TALPHA::__ptrace_newuop(unsigned int iseq,	/* instruction sequence number */
...
/*sim-outorder.cc*/
unsigned int TALPHA::			/* total latency of access */
...
unsigned int TALPHA::		/* latency of block access */
  ...
  if (runcontext->cache_dl2)
    {
      ...
      lat = cache_access(runcontext->cache_dl2, cmd, baddr, NULL, bsize,
...
unsigned int TALPHA::dl2_access_fn(enum mem_cmd cmd,		/* access cmd, Read or Write */
	      md_addr_t baddr,		/* block address to access */
	      int bsize,		/* size of block to access */
	      struct cache_blk_t *blk,	/* ptr to block in upper level */
	      tick_t now)		/* time of access */
...
unsigned int TALPHA::il1_access_fn(enum mem_cmd cmd,		/* access cmd, Read or Write */
	      md_addr_t baddr,		/* block address to access */
	      int bsize,		/* size of block to access */
	      struct cache_blk_t *blk,	/* ptr to block in upper level */
	      tick_t now)		/* time of access */
...
if (runcontext->cache_il2)
    {
      ...
      lat = cache_access(runcontext->cache_il2, cmd, baddr, NULL, bsize,
			 /* now */now, /* pudata */NULL, /* repl addr */NULL);
      if (cmd == Read)
	return lat;
      else
...
unsigned int TALPHA::il2_access_fn(enum mem_cmd cmd,		/* access cmd, Read or Write */
	      md_addr_t baddr,		/* block address to access */
	      int bsize,		/* size of block to access */
	      struct cache_blk_t *blk,	/* ptr to block in upper level */
	      tick_t now)		/* time of access */
...
unsigned int TALPHA::itlb_access_fn(enum mem_cmd cmd,	/* access cmd, Read or Write */
	       md_addr_t baddr,		/* block address to access */
	       int bsize,		/* size of block to access */
	       struct cache_blk_t *blk,	/* ptr to block in upper level */
	       tick_t now)		/* time of access */
...
unsigned int TALPHA::dtlb_access_fn(enum mem_cmd cmd,	/* access cmd, Read or Write */
	       md_addr_t baddr,	/* block address to access */
	       int bsize,		/* size of block to access */
	       struct cache_blk_t *blk,	/* ptr to block in upper level */
	       tick_t now)		/* time of access */
...
void TALPHA::ruu_dump(FILE *stream)				/* output stream */
{
  int num, head;
...
void TALPHA::ruu_dumpent(struct RUU_station *rs,		/* ptr to RUU station */
...
void TALPHA::lsq_dump(FILE *stream)				/* output stream */
{
  int num, head;
  struct RUU_station *rs;
...
void TALPHA::rslink_init(void)			/* total number of RS_LINK available */
{
  int i;
  struct RS_link *link;
...
void TALPHA::ruu_release_fu(void)
{
  int i;
...
void TALPHA::eventq_init(void)
{
  int i;
  for(i = 0; i < MAXTHREADS; i++)
	event_queue = NULL;
}
void TALPHA::readyq_init(void)
{
  int i;
  for(i = 0; i < MAXTHREADS; i++)
	ready_queue = NULL;
}
void TALPHA::eventq_dump(FILE *stream)			/* output stream */
{
  struct RS_link *ev;
  if (!stream)
    stream = stderr;
...
void TALPHA::eventq_queue_event(struct RUU_station *rs, tick_t when)
{
...
struct TALPHA::RUU_station * TALPHA::eventq_next_event(void)
{
  struct RS_link *ev;
...
void TALPHA::readyq_dump(FILE *stream)			/* output stream */
{
  struct RS_link *link;
...
void TALPHA::readyq_enqueue(struct RUU_station *rs)		/* RS to enqueue */
{
...
void TALPHA::cv_dump(FILE *stream)				/* output stream */
{
  int i;
  struct CV_link ent;
		  ...
		  if (runcontext->cache_dl1)
		    {
		      ...
		      lat =
			cache_access(runcontext->cache_dl1, Write, (runcontext->LSQ[runcontext->LSQ_head].addr&~3),
...
void TALPHA::ruu_recover(int branch_index)			/* index of mis-pred branch */
{
  ...
  thispower->gpp_data->core_para->total_branch_miss_num++;
  allpower->gpp_data->core_para->total_branch_miss_num++;
...
void TALPHA::ruu_writeback(void)
{
  int i;
  struct RUU_station *rs;
			      ...
			      == (F_MEM|F_LOAD)))
			{
			  ...
			  if (!load_lat)
			      ...
			      if (runcontext->cache_dl1 && valid_addr)
				{
				  ...
				  load_lat =
				    cache_access(runcontext->cache_dl1, Read,
						 (rs->addr & ~3), NULL, 4,
						 sim_cycle, NULL, NULL);
...
void TALPHA::rspec_dump(FILE *stream)			/* output stream */
{
  int i;
...
void TALPHA::tracer_recover(void)
{
  int i;
...
enum TALPHA::md_fault_type
TALPHA::spec_mem_access(struct mem_t *mem,		/* memory space to access */
...
void TALPHA::mspec_dump(FILE *stream)			/* output stream */
{
  int i;
...
//  return NULL;
//}
void TALPHA::ruu_link_idep(struct RUU_station *rs,		/* rs station to link */
...
void TALPHA::ruu_install_odep(struct RUU_station *rs,	/* creating RUU station */
...
#ifdef TARGET_ALPHA
      runcontext->regs->regs_F.d[MD_REG_ZERO] = 0.0; runcontext->spec_regs_F.d[MD_REG_ZERO] = 0.0;
#endif /* TARGET_ALPHA */
...
#define DEFINST(OP,MSK,NAME,OPFORM,RES,CLASS,O1,O2,I1,I2,I3)		\
	case OP:											\
	  ...
	  /*get res type --lch*/					\
      switch(RES){							\
      case IntALU:		thispower->gpp_data->core_para->total_ialu_num++;allpower->gpp_data->core_para->total_res_call_num++;break;			\
      case RdPort:		thispower->gpp_data->mem_para->total_memport_rd++;allpower->gpp_data->core_para->total_res_call_num++;break;		\
      case WrPort:		thispower->gpp_data->mem_para->total_memport_wt++;allpower->gpp_data->core_para->total_res_call_num++;break;		\
      case IntMULT:							\
      case IntDIV:		thispower->gpp_data->core_para->total_imult_num++;allpower->gpp_data->core_para->total_res_call_num++;break;		\
      case FloatADD:							\
      case FloatCMP:							\
      case FloatCVT:		thispower->gpp_data->core_para->total_fpalu_num++;allpower->gpp_data->core_para->total_res_call_num++;break;		\
      case FloatMULT:							\
      case FloatDIV:							\
      case FloatSQRT:	thispower->gpp_data->core_para->total_fpmult_num++;allpower->gpp_data->core_para->total_res_call_num++;break;		\
      }\
...
#include "alpha.def"
	default:
      ...
      if (!spec_mode && !verbose)
        {
          myfprintf(stderr, "ALPHA --node %d & sim_cycle = %d ++ %10n [xor: 0x%08x] {%d} @ 0x%08p: ",
                    actualid,sim_cycle,sim_num_insn, md_xor_regs(runcontext->regs),
                    inst_seq+1, runcontext->regs->regs_PC);
          fprintf(progout,"ISA: ALP actualid = %d simulation cycle = %d,sim_cycle = %d \t\t\t [xor:0x%08x] {%d} \t\t\t @ 0x%08p",
...
void TALPHA::fetch_dump(FILE *stream)			/* output stream */
{
  int num, head;
	  ...
	  if (runcontext->cache_il1)
	    {
	      /* access the I-cache */
	      lat =
		cache_access(runcontext->cache_il1, Read, IACOMPRESS(runcontext->fetch_regs_PC),
...
/*memory.cc*/
void TALPHA::mem_create(const char *name)			/* name of the memory space */
{
...
byte_t * TALPHA::mem_translate(struct mem_t *mem,	/* memory space to access */
...
void TALPHA::mem_newpage(struct mem_t *mem,		/* memory space to allocate in */
...
enum TALPHA::md_fault_type TALPHA::mem_access(struct mem_t *mem,		/* memory space to access */
...
//void TALPHA::mem_reg_stats(struct mem_t *mem,	/* memory space to declare */
...
void TALPHA::mem_init(void)	/* memory space to initialize */
{
  int i;
...
enum TALPHA::md_fault_type TALPHA::mem_dump(struct mem_t *mem,		/* memory space to display */
...
enum TALPHA::md_fault_type TALPHA::mem_strcpy(mem_access_fn mem_fn,	/* user-specified memory accessor */
	   struct mem_t *mem,		/* memory space to access */
	   enum mem_cmd cmd,		/* Read (from sim mem) or Write */
	   qword_t addr,		/* target address to access */
	   char *s)
...
enum TALPHA::md_fault_type TALPHA::mem_bcopy(mem_access_fn mem_fn,		/* user-specified memory accessor */
	  struct mem_t *mem,		/* memory space to access */
	  enum mem_cmd cmd,		/* Read (from sim mem) or Write */
	  qword_t addr,		/* target address to access */
	  void *vp,			/* host memory address to access */
...
/* copy NBYTES to/from simulated memory space, NBYTES must be a multiple
   ...
   faults encountered */
enum TALPHA::md_fault_type TALPHA::mem_bcopy4(mem_access_fn mem_fn,	/* user-specified memory accessor */
	   struct mem_t *mem,		/* memory space to access */
	   enum mem_cmd cmd,		/* Read (from sim mem) or Write */
	   qword_t addr,		/* target address to access */
	   void *vp,			/* host memory address to access */
...
enum TALPHA::md_fault_type TALPHA::mem_bzero(mem_access_fn mem_fn,		/* user-specified memory accessor */
	  struct mem_t *mem,		/* memory space to access */
	  qword_t addr,		/* target address to access */
	  int nbytes)
{
...
/*loader.cc*/
void TALPHA::ld_reg_stats()	/* stats data base */
{
	statcpnt->stat_reg_addr( "ld_text_base",
		"program text (code) segment base",
		&ld_text_base, ld_text_base, "0x%010p");
	statcpnt->stat_reg_uint( "ld_text_size",
		"program text (code) size in bytes",
		&ld_text_size, ld_text_size, NULL);
	statcpnt->stat_reg_addr( "ld_data_base",
		"program initialized data segment base",
		&ld_data_base, ld_data_base, "0x%010p");
	statcpnt->stat_reg_uint( "ld_data_size",
		"program init'ed `.data' and uninit'ed `.bss' size in bytes",
		&ld_data_size, ld_data_size, NULL);
	statcpnt->stat_reg_addr( "ld_stack_base",
		"program stack segment base (highest address in stack)",
...
#endif
  statcpnt->stat_reg_uint( "ld_stack_size",
		"program initial stack size",
		&ld_stack_size, ld_stack_size, NULL);
  statcpnt->stat_reg_addr( "ld_prog_entry",
		"program entry point (initial PC)",
		&ld_prog_entry, ld_prog_entry, "0x%010p");
  statcpnt->stat_reg_addr( "ld_environ_base",
		"program environment base address address",
		&ld_environ_base, ld_environ_base, "0x%010p");
  statcpnt->stat_reg_int( "ld_target_big_endian",
	       "target executable endian-ness, non-zero if big endian",
	       &ld_target_big_endian, ld_target_big_endian, NULL);
}
void TALPHA::ld_load_prog(char *fname,		/* program to load */
...

  {
	...
	switch (MD_SWAPW(shdr.s_flags))
	  ...
	  case ECOFF_STYP_TEXT:
	    ...
	    mem_bcopy(&TALPHA::mem_access, mem, Write,
	  ...
	  case ECOFF_STYP_FINI:
		...
		/* copy program section into simulator target memory */
		mem_bcopy(&TALPHA::mem_access, mem,
  ...
  for (i=0; i<argc; i++)
    {
      ...
      /* and the data */
      mem_strcpy(&TALPHA::mem_access, mem, Write, sp, argv[i]);
      sp += strlen(argv[i])+1;
    }
  ...
  for (i = 0; envp[i]; i++)
    {
      ...
      mem_access(mem, Write, envp_addr + i*sizeof(md_addr_t),
		 &temp, sizeof(md_addr_t));
      /* and the data */
      mem_strcpy(&TALPHA::mem_access, mem, Write, sp, envp[i]);
      sp += strlen(envp[i]) + 1;
    }
	    ...
	    int traceable)		/* traceable system call? */
{
  ...
  switch (syscode)
    ...
    case OSF_SYS_exit:
      ...
      longjmp(sim_exit_buf,
	      /* exitcode + fudge */(regs->regs_R[MD_REG_A0] & 0xff) + 1);
    		...
    		longjmp(node_exit_buf, /* exitcode + fudge */(regs->regs_R[MD_REG_A0] & 0xff) + 1);
    	}
      break;
    ...
    case OSF_SYS_mpisend:
    {
    	//TODO
    }
    break;
    case OSF_SYS_mpirecv:
    {
    	//TODO
    }
    break;
	...
	if (sim_progfd && MD_OUTPUT_SYSCALL(regs))
	  {
	    ...
	    /*nwritten*/regs->regs_R[MD_REG_V0] =
	      fwrite(buf, 1, /*nbytes*/regs->regs_R[MD_REG_A2], fopen(sim_progfd,"w+"));
	  }
	else
	  {
	...
	buf = (char*)malloc(/* len */(size_t)regs->regs_R[MD_REG_A1]);
	if (!buf)
...
#error No utimes() implementation!
	...
	/* result */regs->regs_R[MD_REG_V0] =
	  connect(/* sockfd */(int)regs->regs_R[MD_REG_A0],
		  (sockaddr*)((void *)&osf_sa), /* addrlen */(int)regs->regs_R[MD_REG_A2]);
	...
	/* allocate host side I/O vectors */
	osf_iov =
	  (struct osf_iovec *)calloc(/* iovcnt */regs->regs_R[MD_REG_A2],
		 sizeof(struct osf_iovec));
	if (!osf_iov)
	  fatal("out of virtual memory in SYS_readv");

	iov =
	  (struct iovec *)calloc(/* iovcnt */regs->regs_R[MD_REG_A2], sizeof(struct iovec));
	if (!iov)
	  fatal("out of virtual memory in SYS_readv");
	...
	if (/* optval */regs->regs_R[MD_REG_A3] != 0
	  ...
	  {
	    buf = (char*)calloc(1, /* optlen */(size_t)regs->regs_R[MD_REG_A4]);
	    if (!buf)
	...
	/* allocate host side I/O vectors */
	fds = (struct pollfd *)calloc(/* nfds */regs->regs_R[MD_REG_A1], sizeof(struct pollfd));
	if (!fds)

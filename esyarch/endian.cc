...
enum endian_t
endian_target_byte_order(TCore * const t)
{
  return t->ld_target_big_endian ? endian_big : endian_little;
}

/* probe target (simulated program) double word endian format,
   only valid after program has been loaded */
enum endian_t
endian_target_word_order(TCore * const t)
{
  /* same as byte sex for SimpleScalar target */
  return endian_target_byte_order(t);
}

    ...
    m_curr_algorithm()
{
    ...
    {
    case RA_XY:    m_curr_algorithm = &EsynetRouter::algorithmXY;    break;
    case RA_TXY:   m_curr_algorithm = &EsynetRouter::algorithmTXY;   break;
    case RA_DYXY:  m_curr_algorithm = &EsynetRouter::algorithmDyXY;  break;
    case RA_TABLE: m_curr_algorithm = &EsynetRouter::algorithmTable; break;
    }
    ...
    for ( long l_port = 0; l_port < m_router_cfg->portNum(); l_port ++ )
    {
        ...
        m_output_port[ l_port ].setOutputNeighbor(
            EsynetVC ( m_router_cfg->port( l_port ).neighborRouter(), m_router_cfg->port( l_port ).neighborPort() ) ); 
        ...
        for ( int i = 0; i < m_network_cfg->routerCount(); i ++ )
        {
            for ( int j = 0; j < m_network_cfg->router( i ).portNum(); j ++ )
            {
                ...
                {
                    m_input_port[ l_port ].setInputNeighbor( EsynetVC ( i, j ) );
                }
            }
        }
        ...
        {
            m_vc_input_arbiter[ l_port ].push_back( EsynetArbiter( esynet::AR_MATRIX, m_router_cfg->totalOutputVc() ) );
        ...
        {
            m_vc_output_arbiter[ l_port ].push_back( EsynetArbiter( esynet::AR_MATRIX, m_router_cfg->totalInputVc() ) );
            m_vc_output_arbiter[ l_port ][ vc ].setInputPortMap( m_network_cfg->router( router_id ) );
        }
        m_port_input_arbiter.push_back( EsynetArbiter( esynet::AR_MATRIX, m_router_cfg->port( l_port ).inputVcNumber() ) );
        m_port_output_arbiter.push_back( EsynetArbiter( esynet::AR_MATRIX, m_router_cfg->totalInputVc() ) );
...
void EsynetRouter::receiveFlit(long phy, long vc, const EsynetFlit & flit)
{
    ...
    if (flit_t.flitType() == EsynetFlit::FLIT_HEAD)
    {
        ...
        if ( m_input_port[ phy ][ vc ].flitCount() == 1 )
        {
            ...
            EsynetVCStatus oldstate = m_input_port[ phy ][ vc ].state();
            m_input_port[ phy ][ vc ].updateState( VCS_ROUTING );
            EsynetVCStatus newstate = m_input_port[ phy ][ vc ].state();
        ...
        for ( long vc_l = 0; vc_l < m_input_port[ pc_l ].vcNumber(); vc_l ++ )
        {
            ...
            if( m_input_port[ pc_l ][ vc_l ].state() == VCS_ROUTING )
            {
                ...
                if ( m_input_port[ pc_l ][ vc_l ].routing().size() > 0 )
                {
                    ...
                    EsynetVCStatus oldstate = m_input_port[ pc_l ][ vc_l ].state();
                    m_input_port[ pc_l ][ vc_l ].updateState( VCS_VC_AB );
                    EsynetVCStatus newstate = m_input_port[ pc_l ][ vc_l ].state();
...
{
    const vector< EsynetVC > & vc_can_t = m_input_port[ a ][ b ].routing();
    long r_size_t = (long)vc_can_t.size();

    for ( long i = 0; i < r_size_t; i ++ )
    {
        EsynetVC v_t = vc_can_t[ i ];
        bool port_avail = false;
        if ( ( m_output_port[ v_t.first ][ v_t.second ].usage() == VC_FREE ) )
        {
            port_avail = true;
        }
    ...
    EsynetVC win = m_vc_input_arbiter[ a ][ b ].grantVc();
...
void EsynetRouter::vcArbitration()
{
    ...
    for ( size_t inphy = 0; inphy < m_input_port.size(); inphy ++ )
    {
        for ( long invc = 0; invc < m_input_port[ inphy ].vcNumber(); invc ++ )
        {
            ...
            if ( m_input_port[ inphy ][ invc ].state() == VCS_VC_AB )
            {
                ...
                {
                    m_vc_output_arbiter[ vc_t.first ][ vc_t.second ].setRequest( EsynetVC ( inphy, invc ) );
    ...
    for ( size_t outphy = 0; outphy < m_output_port.size(); outphy ++ )
    {
                ...
                outvc ++ )
        {
            ...
            {
                EsynetVC vc_win = m_vc_output_arbiter[ outphy ][ outvc ].grantVc();
                if ( vc_win.first >= 0 && vc_win.second >= 0 )
                {
                    EsynetVCStatus oldstate = m_input_port[ vc_win.first ][ vc_win.second ].state();
                    m_input_port[ vc_win.first ][ vc_win.second ].updateState( VCS_SW_AB );
                    EsynetVCStatus newstate = m_input_port[ vc_win.first ][ vc_win.second ].state();
                    ...
                    m_input_port[ vc_win.first ][ vc_win.second ].assignConnection( EsynetVC ( outphy, outvc ) );
...
void EsynetRouter::swArbitration()
{
    for ( size_t inphy = 0; inphy < m_input_port.size(); inphy ++ )
    {
        for( long invc = 0; invc < m_input_port[ inphy ].vcNumber(); invc ++ )
        {
            ...
            {
                EsynetVC out_t = m_input_port[ inphy ][ invc ].connection();
        ...
        if ( win_t >= 0 )
        {
            EsynetVC r_t = m_input_port[ inphy ][ win_t ].connection();
            m_port_output_arbiter[ r_t.first ].setRequest( EsynetVC ( inphy, win_t ) );
        }
    }

    for( size_t outph = 0; outph < m_output_port.size(); outph ++ )
    {
        EsynetVC vc_win = m_port_output_arbiter[ outph ].grantVc();
        if ( vc_win.first >= 0 && vc_win.second >= 0 )
        {
            EsynetVCStatus oldstate = m_input_port[ vc_win.first ][ vc_win.second ].state();
            m_input_port[ vc_win.first ][ vc_win.second ].updateState( VCS_SW_TR );
            EsynetVCStatus newstate = m_input_port[ vc_win.first ][ vc_win.second ].state();
            addEvent( EsynetEvent ( m_current_time, ET_VC_STATE,
                m_router_id, vc_win.first, vc_win.second, m_router_id, vc_win.first, vc_win.second, 
...
void EsynetRouter::flitOutbuffer()
{
    for( size_t phy = 0; phy < m_input_port.size(); phy ++ )
    {
        for( long vc = 0; vc < m_input_port[ phy ].vcNumber(); vc ++ )
        {
            ...
            {
                EsynetVC out_t = m_input_port[ phy ][ vc ].connection();

...
EsynetArbiter::EsynetArbiter( EsynetArbiterType type, int size )
    : m_type( type )
    , m_size( size )
    , m_request( size )
{
...
void EsynetArbiter::setPortMap( const std::vector< esynet::EsynetVC > & port_list )
{
    int seq = 0;
    m_vc_map.clear();
    for ( int phy = 0; phy < port_list.size(); phy ++ )
    {
        m_vc_map[ seq ] = port_list[ phy ];
    }
}

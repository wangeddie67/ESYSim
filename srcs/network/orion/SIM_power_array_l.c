 ...
 */
double SIM_array_dec_report(unsigned long long sim_cycle, SIM_power_dec_t *dec, int telem )
{
  double Etotal;
 ...
 * TODO: add more report functionality, currently only total energy is reported
 */
double SIM_array_wordline_report( unsigned long long sim_cycle, SIM_power_wordline_t *wordline , int telem)
{
...
double SIM_array_bitline_report( unsigned long long sim_cycle, SIM_power_bitline_t *bitline , int telem)
{
...
double SIM_array_amp_report( unsigned long long sim_cycle, SIM_power_amp_t *amp , int telem)
{
...
double SIM_array_comp_report( unsigned long long sim_cycle, SIM_power_comp_t *comp , int telem)
{
...
double SIM_array_mux_report( unsigned long long sim_cycle, SIM_power_mux_t *mux , int telem)
{
...
double SIM_array_outdrv_report( unsigned long long sim_cycle, SIM_power_out_t *outdrv, int telem )
{
...
double SIM_array_mem_report( unsigned long long sim_cycle, SIM_power_mem_t *mem , int telem)
{
...
double SIM_array_pre_report( unsigned long long sim_cycle, SIM_power_arr_pre_t *pre ,int telem)
{
...
double SIM_array_power_report(unsigned long long sim_cycle, SIM_power_array_info_t *info, SIM_power_array_t *arr, int telem)
{
  double epart, etotal = 0;
  
  if (info->row_dec_model) {
    epart = SIM_array_dec_report(sim_cycle, &arr->row_dec, telem);
    //fprintf(stderr, "row decoder: %g\n", epart);
    etotal += epart;
  }
  if (info->col_dec_model) {
    epart = SIM_array_dec_report(sim_cycle, &arr->col_dec, telem);
    //fprintf(stderr, "col decoder: %g\n", epart);
    etotal += epart;
  }
  if (info->data_wordline_model) {
    epart = SIM_array_wordline_report(sim_cycle, &arr->data_wordline, telem);
    //fprintf(stderr, "data wordline: %g\n", epart);
    etotal += epart;
  }
  if (info->tag_wordline_model) {
    epart = SIM_array_wordline_report(sim_cycle, &arr->tag_wordline, telem);
    //fprintf(stderr, "tag wordline: %g\n", epart);
    etotal += epart;
  }
  if (info->data_bitline_model) {
    epart = SIM_array_bitline_report(sim_cycle, &arr->data_bitline, telem);
    //fprintf(stderr, "data bitline: %g\n", epart);
    etotal += epart;
  }
  if (info->data_bitline_pre_model) {
    epart = SIM_array_pre_report(sim_cycle, &arr->data_bitline_pre, telem);
    //fprintf(stderr, "data bitline precharge: %g\n", epart);
    etotal += epart;
  }
  if (info->tag_bitline_model) {
    epart = SIM_array_bitline_report(sim_cycle, &arr->tag_bitline, telem);
    //fprintf(stderr, "tag bitline: %g\n", epart);
    etotal += epart;
  }
  if (info->data_mem_model) {
    epart = SIM_array_mem_report(sim_cycle, &arr->data_mem, telem);
    //fprintf(stderr, "data memory: %g\n", epart);
    etotal += epart;
  }
  if (info->tag_mem_model) {
    epart = SIM_array_mem_report(sim_cycle, &arr->tag_mem, telem);
    //fprintf(stderr, "tag memory: %g\n", epart);
    etotal += epart;
  }
  if (info->data_amp_model) {
    epart = SIM_array_amp_report(sim_cycle, &arr->data_amp, telem);
    //fprintf(stderr, "data amp: %g\n", epart);
    etotal += epart;
  }
  if (info->tag_amp_model) {
    epart = SIM_array_amp_report(sim_cycle, &arr->tag_amp, telem);
    //fprintf(stderr, "tag amp: %g\n", epart);
    etotal += epart;
  }
  if (info->comp_model) {
    epart = SIM_array_comp_report(sim_cycle, &arr->comp, telem);
    //fprintf(stderr, "comparator: %g\n", epart);
    etotal += epart;
  }
  if (info->mux_model) {
    epart = SIM_array_mux_report(sim_cycle, &arr->mux, telem);
    //fprintf(stderr, "multiplexor: %g\n", epart);
    etotal += epart;
  }
  if (info->outdrv_model) {
    epart = SIM_array_outdrv_report(sim_cycle, &arr->outdrv, telem);

  ...
  if ((crsbar->model = model) && model < CROSSBAR_MAX_MODEL) {
    ...
    switch (model) {
      ...
      case MULTREE_CROSSBAR:
	   ...
	   /* redundant field */
       crsbar->depth = (u_int)ceil(log((double)n_in) / log((double)degree));
...
double SIM_crossbar_report(unsigned long long sim_cycle, SIM_power_crossbar_t *crsbar, int telem)
{
...
double SIM_arbiter_report(unsigned long long sim_cycle, SIM_power_arbiter_t *arb, int telem)
{
  switch (arb->model) {
    case MATRIX_ARBITER:
...
int SIM_router_power_report(unsigned long long sim_cycle, SIM_power_router_info_t *info, SIM_power_router_t *router)
{
  double epart, etotal = 0;

  if (info->crossbar_model) {
    epart = SIM_crossbar_report(sim_cycle, &router->crossbar, 1);

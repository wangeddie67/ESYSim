...
private:
    EsynetArbiterType m_type;   /*!< @brief Type of arbiter. */
    int m_size;                 /*!< @brief Number of request signals. */

    std::vector< std::vector< bool > > m_state_matrix;  /*!< @brief Matrix D-FF
        for matrix arbiter. */
    std::vector< bool > m_state_vector;    /*!< @brief Vector D-FF for
        Round-robin arbiter. */

    std::map< int, esynet::EsynetVC > m_vc_map; /*!< @brief Mapping between
        request signals and virtual channel. */
...
public:
     ...
     */
    EsynetArbiter( EsynetArbiterType type, int size );
    /**
     ...
     */
    void setPortMap( const std::vector< esynet::EsynetVC > & port_list );
    /**
     * @}
     */
     ...
     */
    inline void setRequest( const std::vector< bool > & req )
        { m_request = req; }
    /**
     * @brief Clear all request signals.
     */
    inline void clearRequest() 
        { for ( int i = 0; i < m_size; i ++ ) m_request[ i ] = false; }
    /**

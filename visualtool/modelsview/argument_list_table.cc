	...
	case Qt::EditRole:
		{
			...
			case ARGULIST_LOOP:
					 ...
					 t_item->optType() == EsyArgumentItem::ENUM_TYPE )
				{
                    ...
                    {
                        t_str = QString::number( t_item->loop() );
                    }
                    break;
				}
				else
...
Qt::ItemFlags ArgumentListModel::flags( const QModelIndex &index ) const
{
    ...
    else if ( (index.column() == ARGULIST_LOOP && !m_onlyone_model) ||
...
ErrorHandler ArgumentListModel::readArgumentFile( const QString & file )
{
	...
	for ( int argu_index = 0; argu_index < t_argu_list.count(); argu_index ++ )
	{
		...
		{
            int t_loop = t_argu_element.attribute( ARGUMENT_ARGU_LOOP ).toInt();
            if (!m_onlyone_model)
            {
                t_loop = 0;
            }
			appendItem( new ArgumentListModelItem(
				...
				t_argu_element.attribute( ARGUMENT_ARGU_NOTE ).
					replace( "\\n", "\n" ),
                t_argu_element.attribute( ARGUMENT_ARGU_VALUE ), t_loop,
...
ArgumentListTableView::ArgumentListTableView(bool onlyone, QWidget *parent) :
    QTableView( parent ), m_onlyone_model( onlyone )
{
...
void ArgumentListTableView::updateColumnWidth()
{
    if (m_onlyone_model)
    {
        setColumnHidden( ArgumentListModel::ARGULIST_LOOP, true );
    }

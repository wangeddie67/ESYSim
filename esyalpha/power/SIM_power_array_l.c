 ...
 */
double SIM_array_dec_report( SIM_power_dec_t *dec, int telem )
{
    double Etotal;

    Etotal = telem * sim_cycle * turnoff_factor * (dec->e_chg_output + dec->e_chg_l1 + dec->e_chg_addr)
             + dec->n_chg_output * dec->e_chg_output + dec->n_chg_l1 * dec->e_chg_l1 +
...
double SIM_Array_Dec_Report( SIM_power_dec_t *dec )
{
    ...
    Etotal = turnoff_factor * (dec->e_chg_output + dec->e_chg_l1 + dec->e_chg_addr)
             + dec->n_chg_output * dec->e_chg_output + dec->n_chg_l1 * dec->e_chg_l1 +
             dec->n_chg_addr * dec->e_chg_addr;
    dec->n_chg_output = dec->n_chg_l1 = dec->n_chg_addr = 0;
 ...
 * TODO: add more report functionality, currently only total energy is reported
 */
double SIM_array_wordline_report( SIM_power_wordline_t *wordline , int telem)
{
    return (telem * sim_cycle * turnoff_factor * (wordline->e_read + wordline->e_write)
            +  wordline->n_read * wordline->e_read +
...
double SIM_Array_Wordline_Report( SIM_power_wordline_t *wordline )
{
    ...
    eword = turnoff_factor * (wordline->e_read + wordline->e_write)
            +  wordline->n_read * wordline->e_read +
            wordline->n_write * wordline->e_write ;
    wordline->n_read = wordline->n_write = 0;
...
double SIM_array_bitline_report( SIM_power_bitline_t *bitline , int telem)
{
    return (telem * sim_cycle * turnoff_factor * (bitline->e_col_write +
            bitline->e_col_read + bitline->e_col_sel) +
            bitline->n_col_write * bitline->e_col_write +
...
double SIM_Array_Bitline_Report( SIM_power_bitline_t *bitline )
{
    ...
    ebitline = turnoff_factor * (bitline->e_col_write +
                                 bitline->e_col_read + bitline->e_col_sel) +
               bitline->n_col_write * bitline->e_col_write +
               bitline->n_col_read * bitline->e_col_read +
               bitline->n_col_sel * bitline->e_col_sel ;
...
double SIM_array_amp_report( SIM_power_amp_t *amp , int telem)
{
    return (telem * sim_cycle * turnoff_factor * amp->e_access + amp->n_access * amp->e_access );
}
...
double SIM_Array_Amp_Report( SIM_power_amp_t *amp )
{
    ...
    eamp = turnoff_factor * amp->e_access + amp->n_access * amp->e_access ;
    amp->n_access = 0;
...
double SIM_array_comp_report( SIM_power_comp_t *comp , int telem)
{
    return (telem * sim_cycle * turnoff_factor * (comp->e_access + comp->e_match +
            comp->e_mismatch + comp->e_miss + comp->e_bit_match + comp->e_chg_addr + comp->e_bit_mismatch) +
            comp->n_access * comp->e_access + comp->n_match * comp->e_match +
...
double SIM_Array_Comp_Report( SIM_power_comp_t *comp )
{
    ...
    ecomp = turnoff_factor * (comp->e_access + comp->e_match +
                              comp->e_mismatch + comp->e_miss + comp->e_bit_match + comp->e_chg_addr + comp->e_bit_mismatch) +
            comp->n_access * comp->e_access + comp->n_match * comp->e_match +
            comp->n_mismatch * comp->e_mismatch + comp->n_miss * comp->e_miss +
            comp->n_bit_match * comp->e_bit_match + comp->n_chg_addr * comp->e_chg_addr +
            comp->n_bit_mismatch * comp->e_bit_mismatch ;
    comp->n_access = comp->n_match = comp->n_mismatch = comp->n_miss = 0;
    comp->n_bit_match = comp->n_chg_addr = comp->n_bit_mismatch = 0;
...
double SIM_array_mux_report( SIM_power_mux_t *mux , int telem)
{
    return (telem * sim_cycle * turnoff_factor * (mux->e_mismatch + mux->e_chg_addr) +
            mux->n_mismatch * mux->e_mismatch + mux->n_chg_addr * mux->e_chg_addr );
}
...
double SIM_Array_Mux_Report( SIM_power_mux_t *mux )
{
    ...
    emux = turnoff_factor * (mux->e_mismatch + mux->e_chg_addr) +
           mux->n_mismatch * mux->e_mismatch + mux->n_chg_addr * mux->e_chg_addr ;
    mux->n_mismatch = mux->n_chg_addr = 0;
...
double SIM_array_outdrv_report( SIM_power_out_t *outdrv, int telem )
{
    return (telem * sim_cycle * turnoff_factor * (outdrv->e_select + outdrv->e_chg_data + outdrv->e_out_1 + outdrv->e_out_0) +
            outdrv->n_select * outdrv->e_select + outdrv->n_chg_data * outdrv->e_chg_data +
...
{
    double eout;
    eout = turnoff_factor * (outdrv->e_select + outdrv->e_chg_data + outdrv->e_out_1 + outdrv->e_out_0) +
           outdrv->n_select * outdrv->e_select + outdrv->n_chg_data * outdrv->e_chg_data +
           outdrv->n_out_1 * outdrv->e_out_1 + outdrv->n_out_0 * outdrv->e_out_0 ;
    outdrv->n_select = outdrv->n_chg_data = 0;
    outdrv->n_out_0 = outdrv->n_out_1 = 0;
...
double SIM_array_mem_report( SIM_power_mem_t *mem , int telem)
{
    return (telem * sim_cycle * turnoff_factor * mem->e_switch + mem->n_switch * mem->e_switch );
}

double SIM_Array_Mem_Report( SIM_power_mem_t *mem )
{
    double emem;
    emem = turnoff_factor * mem->e_switch + mem->n_switch * mem->e_switch ;
    mem->n_switch = 0;
...
double SIM_array_pre_report( SIM_power_arr_pre_t *pre ,int telem)
{
    return (telem * sim_cycle * turnoff_factor * pre->e_charge +  pre->n_charge * pre->e_charge );
}
...
double SIM_Array_Pre_Report( SIM_power_arr_pre_t *pre )
{
    ...
    epre = turnoff_factor * pre->e_charge +  pre->n_charge * pre->e_charge ;
    pre->n_charge = 0;
...
double SIM_array_power_report(SIM_power_array_info_t *info, SIM_power_array_t *arr, int telem)
{
    double epart, etotal = 0;

    if (info->row_dec_model)
    {
        epart = SIM_array_dec_report(&arr->row_dec, telem);
        //fprintf(stderr, "row decoder: %g\n", epart);
        etotal += epart;
    }
    if (info->col_dec_model)
    {
        epart = SIM_array_dec_report(&arr->col_dec, telem);
        //fprintf(stderr, "col decoder: %g\n", epart);
        etotal += epart;
    }
    if (info->data_wordline_model)
    {
        epart = SIM_array_wordline_report(&arr->data_wordline, telem);
        //fprintf(stderr, "data wordline: %g\n", epart);
        etotal += epart;
    }
    if (info->tag_wordline_model)
    {
        epart = SIM_array_wordline_report(&arr->tag_wordline, telem);
        //fprintf(stderr, "tag wordline: %g\n", epart);
        etotal += epart;
    }
    if (info->data_bitline_model)
    {
        epart = SIM_array_bitline_report(&arr->data_bitline, telem);
        //fprintf(stderr, "data bitline: %g\n", epart);
        etotal += epart;
    }
    if (info->data_bitline_pre_model)
    {
        epart = SIM_array_pre_report(&arr->data_bitline_pre, telem);
        //fprintf(stderr, "data bitline precharge: %g\n", epart);
        etotal += epart;
    }
    if (info->tag_bitline_model)
    {
        epart = SIM_array_bitline_report(&arr->tag_bitline, telem);
        //fprintf(stderr, "tag bitline: %g\n", epart);
        etotal += epart;
    }
    if (info->data_mem_model)
    {
        epart = SIM_array_mem_report(&arr->data_mem, telem);
        //fprintf(stderr, "data memory: %g\n", epart);
        etotal += epart;
    }
    if (info->tag_mem_model)
    {
        epart = SIM_array_mem_report(&arr->tag_mem, telem);
        //fprintf(stderr, "tag memory: %g\n", epart);
        etotal += epart;
    }
    if (info->data_amp_model)
    {
        epart = SIM_array_amp_report(&arr->data_amp, telem);
        //fprintf(stderr, "data amp: %g\n", epart);
        etotal += epart;
    }
    if (info->tag_amp_model)
    {
        epart = SIM_array_amp_report(&arr->tag_amp, telem);
        //fprintf(stderr, "tag amp: %g\n", epart);
        etotal += epart;
    }
    if (info->comp_model)
    {
        epart = SIM_array_comp_report(&arr->comp, telem);
        //fprintf(stderr, "comparator: %g\n", epart);
        etotal += epart;
    }
    if (info->mux_model)
    {
        epart = SIM_array_mux_report(&arr->mux, telem);
        //fprintf(stderr, "multiplexor: %g\n", epart);
        etotal += epart;
    }
    if (info->outdrv_model)
    {
        epart = SIM_array_outdrv_report(&arr->outdrv, telem);

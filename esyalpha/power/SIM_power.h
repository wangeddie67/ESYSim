...
#define HAMM_MASK(w)	(w < (sizeof(LIB_Type_max_uint) << 3) ? (BIGONE << w) - 1 : BIGNONE)
 ...
 */
#if ( PARM(TECH_POINT) == 045 )
#define CSCALE		(470.7)	/* wire capacitance scaling factor */
#define RSCALE		(437.0)	/* wire resistance scaling factor */
#define LSCALE		0.05625		/* length (feature) scaling factor */
#define ASCALE		(LSCALE*LSCALE)	/* area scaling factor */
#define VSCALE		0.2		/* voltage scaling factor */
#define VTSCALE		0.13761		/* threshold voltage scaling factor */
#define SSCALE		0.764		/* sense voltage scaling factor */

#define MCSCALE		7.5//5.2277		/* metal coupling capacitance scaling factor */
#define MCSCALE2	5.22//3		/* metal coupling capacitance scaling factor (2X) */
#define MCSCALE3	3//1.5		/* metal coupling capacitance scaling factor (3X) */

#define GEN_POWER_SCALE (1/(PARM(GEN_POWER_FACTOR)*PARM(GEN_POWER_FACTOR)*PARM(GEN_POWER_FACTOR)))
...
#define Energyfactor	(PARM(Freq))*Vdd*Vdd
#define SenseEnergyfactor (PARM(Freq))*Vdd*(Vdd/2)

extern double EnergyFactor;
extern double SenseEnergyFactor;
extern double EnergyBack;
extern double SenseEnergyBack ;

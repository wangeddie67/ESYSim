...
double SIM_bus_report(SIM_power_bus_t *bus, int telem)
{
    extern unsigned long long sim_cycle;
    return (telem*2*sim_cycle * turnoff_factor * bus->e_switch + bus->n_switch * bus->e_switch);
}
...
double SIM_Bus_Report(SIM_power_bus_t *bus)
{
    ...
    bus_power = 2 * turnoff_factor * bus->e_switch + bus->n_switch * bus->e_switch;
    bus->n_switch = 0;
    return (bus_power);
}
...
{
    double eff;
    eff = ff->e_switch * ff->n_switch + ff->e_clock * ff->n_clock +
          ff->e_keep_0 * ff->n_keep_0 + ff->e_keep_1 * ff->n_keep_1;
    ff->n_switch = ff->n_clock = ff->n_keep_0 = ff->n_keep_1 = 0;

...
#ifdef DIRECTORY
#ifdef ESYNET
	extern int esynetInit();
	extern int esynetMsgComplete(long w, long x, long y, long z, long 
long int stTime, long long int msgNo);
	extern int esynetBufferSpace(long s1, long s2, int opt);
#endif
	...
	counter_t totalOptHopCount, totalOptHopDelay;
...
#ifdef	EDA
/* level 0 data cache */
struct cache_t *cache_dl0[THREADS_PER_JOB];

counter_t recoveryCount[8];
counter_t numBCQEmpty[8];

/* Filter cache (dl0) cleanup at regular intervals */
counter_t intervalCount[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };
counter_t totalRelCE[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };
counter_t nonSktInsn[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };
counter_t numSyncInst[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };
counter_t pure_sync_insn = 0;
counter_t pure_skt_insn = 0;
...
dl0_cleanup ()
{
	...
	{
		context *current = thecontexts[i];
		int jobId = current->jobThdId;

		if (current->masterid != 0)
			continue;

		if (current->sim_num_insn > intervalCount[jobId])
		{
			flushSelective (cache_dl0[jobId], jobId);
			intervalCount[jobId] += DL0_CLEAUP_INTERVAL;
		}
	}
}

/* Oracle Store Queue */
struct oracle_store_queue OSQ[8][OSQ_size];
int OSQ_num[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };
int OSQ_head[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };

	int
findOSQ (md_addr_t addr, int width, int id)
{
	int i, j;

	for (i = (OSQ_head[id] + OSQ_size - 1) % OSQ_size, j = OSQ_num[id]; j > 0; j--, i = (i + OSQ_size - 1) % OSQ_size)
	{
		int bigWidth = OSQ[id][i].memWidth > width ? OSQ[id][i].memWidth : width;
		md_addr_t lAddr = addr & ~(bigWidth - 1);
		md_addr_t osqAddr = OSQ[id][i].memAddr & ~(bigWidth - 1);
...
md_addr_t prefQueue[MAXTHREADS][PREFETCH_QUEUE_SIZE];
md_addr_t prefQueue_pc[MAXTHREADS][PREFETCH_QUEUE_SIZE];
counter_t prefQueue_instNum[MAXTHREADS][PREFETCH_QUEUE_SIZE];
int prefQueue_isWrongPath[MAXTHREADS][PREFETCH_QUEUE_SIZE];
int prefQueue_head[MAXTHREADS] = 0;
int prefQueue_tail[MAXTHREADS] = 0;
int prefQueue_size[MAXTHREADS] = 0;
int memMissCap = 0;
counter_t totalPrefMissLatency = 0;
int prefMissCount = 0;
counter_t decpPrefStallCount = 0;
md_addr_t lastPrefAddr = 0;
#endif
...
counter_t data_packets_sending[100][100];
counter_t meta_packets_sending[100][100];
counter_t link_ser_lat[100][100];
counter_t meta_even_req_close[36];
counter_t meta_odd_req_close[36];
...
mem_access_latency (int blk_sz)	/* block size accessed */
{
	...
	return ( /* first chunk latency */ mem_lat[0] +
			( /* remainder chunk latency */ ((float)5/mem_bus_speed) * (chunks - 1)));
...
#ifdef DIRECTORY
		lat = cache_access (cache_dl2, cmd, baddr, NULL, bsize, /*now*/now, /*pudata*/ NULL, /*repl addr*/NULL, /*rs pointer*/NULL, /*threadid */which_thread);
#else
		lat = cache_access (cache_dl2, cmd, baddr, NULL, bsize, /* now */ now, /* pudata */ NULL, /*repl addr*/ NULL, /*threadid */ which_thread);
#endif
	      ...
	      )
{
	...
	//if (cmd == Read)
	{
		...
		mem_port_lat[port_num] += (bsize*mem_port_num/mem_bus_width)*((float)5/mem_bus_speed);  //mem bus speed is 1.25GHz while CPU is 5GHz
#else
...
#ifdef DIRECTORY
				/* ruu writewait pointer */NULL,
#endif
				threadid IF_L2_COMM_LAT_CL (cluster));
...
sim_reg_options (struct opt_odb_t *odb)
{
...
#ifdef	EDA
	opt_reg_string (odb, "-cache:dl0", "l0 data cache config, i.e., {<config>|none} (each cluster)", &cache_dl0_opt, "dl0:128:32:4:l",
			/* print */ TRUE, NULL);

	opt_reg_int (odb, "-cache:dl0lat", "l0 data cache hit latency (in cycles)", &cache_dl0_lat, /* default */ 1,
			/* print */ TRUE, /* format */ NULL);
#endif
	...
	opt_reg_int (odb, "-mem:width", "memory access bus width (in bytes)", &mem_bus_width, /* default */ 8,
			/* print */ TRUE, /* format */ NULL);
	opt_reg_float (odb, "-mem:speed", "memory access bus speed (in GHz)", &mem_bus_speed, /* default */ 1.25,
			/* print */ TRUE, /* format */ NULL);
	...
	opt_reg_int (odb, "-maxThrds", "Maximum Number of Threads Allowed", &mta_maxthreads, 1, TRUE, NULL);

	opt_reg_flag (odb, "-Coherent_Cache", "Coherent Cache", &COHERENT_CACHE, 1, TRUE, NULL);

	opt_reg_flag (odb, "-MSI_prot", "MSI Protocol", &MSI, 0, TRUE, NULL);

	opt_reg_flag (odb, "-SPEC", "SPEC Benchmarks", &spec_benchmarks, 1, TRUE, NULL);
	...
	opt_reg_flag (odb, "-bugcompat", "operate in backward-compatible bugs mode (for testing only)", &bugcompat_mode, /* default */ FALSE, /* print */ TRUE, NULL);
	...
	opt_reg_int (sim_odb, "-hotspot:sampling_intvl", "how many cycles between calls to update_hotspot_stats", &hotspot_cycle_sampling_intvl,
			/* default */ 10000, /* !print */ FALSE, NULL);
		...
		int argc, char **argv)	/* command line arguments */
{
	...
	{
		if (sscanf (cache_dl0_opt, "%[^:]:%d:%d:%d:%c", name, &nsets, &bsize, &assoc, &c) != 5)
			fatal ("bad l1 D-cache parms: <name>:<nsets>:<bsize>:<assoc>:<repl>");

		for (i = 0; i < THREADS_PER_JOB; i++)
		{
			cache_dl0[i] = cache_create (name, nsets, bsize, /* balloc */ TRUE,
					/* usize */ 0, assoc, cache_char2policy (c),
					dl0_access_fn, /* hit lat */ cache_dl0_lat, i);
#ifdef CACHE_SET_STAT
			if (cache_dl0[i]->nsets > MAXCACHESET)
...
#ifdef	EDA
	stat_reg_counter (sdb, "pure_sync_insn", "total number of main thread sync instructions committed", &pure_sync_insn, pure_sync_insn, NULL);
	stat_reg_counter (sdb, "pure_skt_insn", "total number of main thread skt instructions committed", &pure_skt_insn, pure_skt_insn, NULL);
#endif
...
#ifdef	EDA
		sprintf (ch2, "nonSktInsn_%d", i);
		stat_reg_counter (sdb, ch2, "Total Non skeleton instructions executed", &nonSktInsn[i], /* initial value */ 0, /* format */ NULL);
		sprintf (ch2, "recoveryCount_%d", i);
		stat_reg_counter (sdb, ch2, "Recovery count", &recoveryCount[i], /* initial value */ 0, /* format */ NULL);
		sprintf (ch2, "stallCC_BOQEmpty_%d", i);
		stat_reg_counter (sdb, ch2, "Correctness core fetch stall due to BOQ empty", &numBCQEmpty[i], /* initial value */ 0, /* format */ NULL);
		sprintf (ch2, "totalRelCE_%d", i);
		stat_reg_counter (sdb, ch2, "Total Dl0 cache evictions.", &totalRelCE[i], /* initial value */ 0, /* format */ NULL);
		sprintf (ch2, "numSyncInst_%d", i);
		stat_reg_counter (sdb, ch2, "Total num of synchronization instructions", &numSyncInst[i], /* initial value */ 0, /* format */ NULL);
#endif
	}
...
#ifdef DIRECTORY
	dir_eventq_init();
	for(j = 0; j < numcontexts+mesh_size*2; j++)
	{
		dir_fifo_head[j] = 0;
		dir_fifo_tail[j] = 0;
		dir_fifo_num[j] = 0;
...
int buffer_full_check(struct DIRECTORY_EVENT *event)
{
	...
	if((event->des1*mesh_size+event->des2 != event->src1*mesh_size+event->src2) && (event->input_buffer_full) && (event->operation == MISS_WRITE || event->operation == MISS_READ || event->operation == WRITE_UPDATE || event->operation == MISS_IL1))
	{
		...
		else if (!mystricmp (network_type, "MESH"))
		{
            ...
            vc = esynetBufferSpace(event->src1, event->src2, options);
			if(vc == -1)
				buffer_full = 1;
		...
		{
			/* fixme: combination should check the buffer depends on the location */
...
#endif
#ifdef OPTICAL_INTERCONNECT
				opticalBufferSpace(event->src1, event->src2, event->operation)
#endif
		  )
#endif
		...
		if (event->when <= sim_cycle)
		{
			...
			if(insert_flag == 0)
			{
				if(event == dir_event_queue)
				{
					...
					{
						if(event->pending_flag == 0)
						{
							event->pending_flag = 1;
							mshr_pending_event[(event->src1-MEM_LOC_SHIFT)*mesh_size + event->src2] ++;
						}
...
ruu_commit (void)
{
	...
	{
#ifdef	EDA
		if (thecontexts[thread_commit]->masterid == 2)
			continue;
#endif
		if (j > 0)
		{
...
#ifdef	EDA
			if ((current->masterid == 1) && (MD_OP_FLAGS (rs->op) & F_CTRL) && queueSize (current->predQueue) == -1)
				break;
#endif
#ifdef  DIRECTORY
			if(rs->writewait == 1 && !mshr_pending_event[j])
			{
				break;
			}
#endif
...
#endif
								{
...
#ifdef SEQUENTIAL_CONSISTENCY
#ifdef	EDA
									if (collect_stats && current->masterid == 0)
#else
										if (collect_stats)
#endif
										{
											...
											for (bx = 0; bx < numcontexts; bx++)
											{
												bool = 0;

												tmpCxt = thecontexts[bx];
#ifdef	EDA
												if (tmpCxt->masterid != 0)
													continue;
#endif

												if (tmpCxt != current)
												{
													for (cx = tmpCxt->LSQ_head, dx = 0; dx != tmpCxt->LSQ_num; cx = (cx + 1) % LSQ_size, dx++)
													{
														if ((MD_OP_FLAGS (tmpCxt->LSQ[cx].op) & (F_MEM | F_LOAD)) == (F_MEM | F_LOAD))
														{
															if ((tmpCxt->LSQ[cx].addr & mask) == (current->LSQ[LSQ_index].addr & mask))
															{
																if (!tmpCxt->LSQ[cx].spec_mode && !tmpCxt->LSQ[cx].isPrefetch)
																{
																	//need to fix the state to non spec mode if in spec mode
																	fixSpecState (tmpCxt->LSQ[cx].prod[1]->index, tmpCxt->id);
																	tmpCxt->regs = tmpCxt->LSQ[cx].prod[1]->backupReg;
																	seqConsistancyReplay (tmpCxt->LSQ[cx].prod[1]->index, tmpCxt->id);
																	invalidation_replay[bx]++;
																	break;
																}
															}
														}
												...
												if (tmpCxt != current)
												{
													for (cx = tmpCxt->LSQ_head, dx = 0; dx != tmpCxt->LSQ_num; cx = (cx + 1) % LSQ_size, dx++)
													{
														if ((MD_OP_FLAGS (tmpCxt->LSQ[cx].op) & (F_MEM | F_LOAD)) == (F_MEM | F_LOAD))
														{
															if ((tmpCxt->LSQ[cx].addr & mask) == (current->LSQ[LSQ_index].addr & mask))
															{
																if (!tmpCxt->LSQ[cx].spec_mode && tmpCxt->LSQ[cx].issued)
																{
																	tmpCxt->LSQ[cx].invalidationReceived = 1;
																}
															}
														}
													}
									...
									if (cache_dl1[threadid])
									{
										...
										if (COHERENT_CACHE && allForked)
										{
											...
											{
												lat = cache_access(cache_dl1[threadid], Write, (current->LSQ[LSQ_index].addr&~3), NULL, 4, sim_cycle, NULL, NULL,/* RUU pointer */ rs, threadid);  
												if (lat<WAIT_TIME)
												{
													completeStore(threadid, current->LSQ[LSQ_index].addr, GetMemAccWidth(current->LSQ[LSQ_index].op), 0);
												}
...
#else
										MSHRLookup(cache_dl1[threadid]->mshr, current->LSQ[LSQ_index].addr, lat, 0, NULL);
#endif
#endif
										if (lat >= WAIT_TIME)
										{
...
#ifdef	EDA
											{
												int wL = GetMemAccWidth (current->LSQ[LSQ_index].op);
												qword_t wD = 0;
												formatDataR2M (current->LSQ[LSQ_index].op, current->LSQ[LSQ_index].val_ra, &wD);

												rsCacheAccess = current->LSQ[LSQ_index];
												lat = cache_access (cache_dl1[threadid], Write, (current->LSQ[LSQ_index].addr & ~(wL - 1)), &wD, wL, sim_cycle, NULL, NULL, threadid);
											}
#else
											lat = cache_access (cache_dl1[threadid], Write, (current->LSQ[LSQ_index].addr & ~3), NULL, 4, sim_cycle, NULL, NULL, threadid);
#endif
										}
										else
										{
		...
		if (RSLINK_VALID (node))
		{
			...
			if (((rs->when_ready[0] <= sim_cycle) && (rs->when_ready[1] <= sim_cycle) && (rs->when_ready[2] <= sim_cycle) && (rs->when_inq < sim_cycle) && !rs->in_qwait))
			{
				...
				if (rs->in_LSQ && ((MD_OP_FLAGS (rs->op) & (F_MEM | F_STORE)) == (F_MEM | F_STORE)))
				{
...
#ifdef	EDA
					if (rs->poison[1].dirty == 1)
						addr = rs->poison[1].data + SEXT (rs->IR & 0xffff);
					else
#endif
						addr = rs->addr;
					...
					{
						tlb_lat = 0;
						tlb_lat = cache_access (dtlb[threadid], Read, (addr & ~3), NULL, 4, sim_cycle, NULL, NULL, NULL, threadid);
...
#ifdef	EDA
					int isDL0Hit = 1;
					if(current->masterid == 1 && (isDL0Hit = isCacheHit(cache_dl0[current->jobThdId], addr, threadid)) == 0  && isMSHRFull(cache_dl1[current->helpThdId]->mshr, 1))
#else
						if (isMSHRFull(cache_dl1[threadid]->mshr, 0, threadid) && !mshr_pending_event[threadid])
#endif
						{
							readyq_enqueue (rs);
							RSLINK_FREE (node);
					...
					if (valid_addr && (rs->op != STL_C))
					{
						...
						{
							if (cache_dl1[threadid])
								lat = cache_access (cache_dl1[threadid], Write, (addr & ~3), NULL, 4, sim_cycle, NULL, NULL, 
#ifdef DIRECTORY
										rs,
#endif	
										rs->threadid);
...
#else
							MSHRLookup(cache_dl1[threadid]->mshr, addr, lat, 0, NULL);
#endif
#endif
						}
						else
					...
					if (MD_OP_FUCLASS (rs->op) != NA)
					{
						...
						if (fu)
						{
							...
							if (rs->in_LSQ && ((MD_OP_FLAGS (rs->op) & (F_MEM | F_LOAD)) == (F_MEM | F_LOAD)))
							{
								...
								{
									tlb_lat = 0;
									tlb_lat = cache_access(dtlb[threadid], Read, (addr & ~3), NULL, 4, sim_cycle, NULL, NULL, NULL, rs->threadid);  // add a parameter here
								...
								int matchnum;
								matchnum = MSHR_block_check(cache_dl1[threadid]->mshr, addr, cache_dl1[threadid]->set_shift);
								if(matchnum && !rs->isPrefetch && !mshr_pending_event[threadid])
								{
									cache_dl1[threadid]->in_mshr ++;
									rs->cache_ready = 1;
									fu->master->busy = 0;
...
#ifdef	EDA
								int isDL0Hit = 1;
								if((current->masterid == 0 && isMSHRFull(cache_dl1[threadid]->mshr, 0)) || 
										(current->masterid == 1 && (isDL0Hit = isCacheHit(cache_dl0[current->jobThdId], addr, threadid)) == 0  && isMSHRFull(cache_dl1[current->helpThdId]->mshr, 1)))
#else
								...
								if (cache_dl1[threadid] && valid_addr && !(matchnum))
								{
									...
									rsCacheAccess = rs;
#ifdef DIRECTORY
									load_lat = cache_access(cache_dl1[threadid], Read, (addr & ~3), NULL, 4, sim_cycle, NULL, NULL, /*ruu pointer*/rs, rs->threadid); 
#else
									load_lat = cache_access (cache_dl1[threadid], Read, (addr & ~3), NULL, 4, sim_cycle, NULL, NULL, rs->threadid);
#endif
...
#else
								eventq_queue_event (rs, sim_cycle + load_lat + 5);
#endif
								else
									/* use computed cache access latency */
#ifdef DIRECTORY
...
#ifdef	EDA
				if (current->masterid == 1)
				{
					if (rs->poison[0].dirty || rs->poison[1].dirty)
					{
						if (rs->ea_comp)
						{
							rs->poison[2].dirty = 1;
							rs->poison[2].data = rs->poison[1].data + SEXT (rs->IR & 0xffff);
						}
						else if (!rs->in_LSQ)
							calculateWrongValue (rs);
					}
					else if (!(MD_OP_FLAGS (rs->op) & F_LOAD))
						rs->poison[2].dirty = 0;
				}
#endif
			}
...
#include "machine.def"
				...
				if (op != MD_NOP_OP)
				{
					...
					rs->isSyncInst = (collect_lock_stats[threadid] | collect_barrier_stats[threadid]);
...
#if	defined(EDA) & defined(RUN_ORACLE)
					if (current->masterid == 1)
						rs->sim_num_insn = thecontexts[current->oracleId]->sim_num_insn;
					else
#endif
					...
					if ((MD_OP_FLAGS (op) & F_MEM) && second_run)
					{
						...
						lsq->isSyncInst = (collect_lock_stats[threadid] | collect_barrier_stats[threadid]);
...
#if	defined(EDA) & defined(RUN_ORACLE)
						if (current->masterid == 1)
							lsq->sim_num_insn = thecontexts[current->oracleId]->sim_num_insn;
						else
#endif
	...
	while (tID < numcontexts)
	{
				...
				&& !done && !current->ruu_fetch_issue_delay && current->running && j < (ruu_decode_width * fetch_speed) && current->active_this_cycle /*&& ((ray_mult * (j + current->fetch_num) + current->RUU_num) < (RUU_size - 1))*/; i++, j++)
		{
			...
			if (current->ld_text_base <= current->fetch_regs_PC && current->fetch_regs_PC < (current->ld_text_base + current->ld_text_size) && !(current->fetch_regs_PC & (sizeof (md_inst_t) - 1)))
			{
				...
				if(current->masterid == 1)
				{
					MD_FETCH_INST (inst, thecontexts[current->helpThdId]->mem, current->fetch_regs_PC);
				}
				else
#endif
				{
				...
				if (!current->wait_for_fetch)
				{
					...
					if (cache_il1[threadid])
					{
						...
						lat = cache_access (cache_il1[threadid], Read, IACOMPRESS(current->fetch_regs_PC),
								...
								NULL, NULL, /*ruu writewait*/NULL,threadid); 
#else
						lat = cache_access (cache_il1[threadid], Read, IACOMPRESS (current->fetch_regs_PC), NULL, ISCOMPRESS (sizeof (md_inst_t)), sim_cycle, NULL, NULL, threadid);
#endif
					...
					if (itlb[threadid])
					{
						...
						tlb_lat = cache_access(itlb[threadid], Read, IACOMPRESS(current->fetch_regs_PC),
								NULL, ISCOMPRESS(sizeof(md_inst_t)), sim_cycle,
								NULL, NULL, /*ruu writewait*/NULL, threadid); /* Note: Assumes FE is at cluster zero. This isn't used anyways though. */ 
#else
						tlb_lat = cache_access (itlb[threadid], Read, IACOMPRESS (current->fetch_regs_PC), NULL, ISCOMPRESS (sizeof (md_inst_t)), sim_cycle, NULL, NULL, threadid);	/* Note: Assumes FE is at cluster zero. This isn't used anyways though. */
#endif
...
sim_main (void)
{
	...
	for (;;)
	{
		...
		for (threadid = 0; threadid < numcontexts; threadid++)
		{
			for (loopiter = 0; loopiter < 20; loopiter++)
			{
				counter_t icount;
				md_inst_t inst;	/* actual instruction bits */
				enum md_opcode op;	/* decoded opcode enum */
				...
				//printf("%d, %d, %lx\n", numcontexts, threadid, current->regs.regs_PC);
...
#include "machine.def"
				...
				if (fault != md_fault_none)
					fatal ("non-speculative fault (%d) detected @ 0x%08p", fault, current->regs.regs_PC);
				...
				if (MD_OP_FLAGS (op) & F_LOAD)
				{
					int lat;
#ifdef DIRECTORY
					int load_lat = cache_access (cache_dl1[threadid], Read, (addr & ~3), NULL, 4, sim_num_insn, NULL, NULL, rs, 0);
#else
					int load_lat = cache_access (cache_dl1[threadid], Read, (addr & ~3), NULL, 4, sim_num_insn, NULL, NULL, 0);
#endif
					if (dtlb)
#ifdef DIRECTORY
						lat = cache_access (dtlb[threadid], Read, (addr & ~3), NULL, 4, sim_num_insn, NULL, NULL, NULL, 0);
#else
					lat = cache_access (dtlb[threadid], Read, (addr & ~3), NULL, 4, sim_num_insn, NULL, NULL, 0);
#endif
					load_lat = max2 (load_lat, lat);
					if (load_lat > 50)
						PushMissPredLoad (current->regs.regs_PC, (int) load_lat);
				}
				else if (MD_OP_FLAGS (op) & F_STORE)
				{
#ifdef DIRECTORY
					cache_access (cache_dl1[threadid], Write, (addr & ~3), NULL, 4, sim_num_insn, NULL, NULL, rs, 0);
#else
					cache_access (cache_dl1[threadid], Write, (addr & ~3), NULL, 4, sim_num_insn, NULL, NULL, 0);
#endif
					if (dtlb)
#ifdef DIRECTORY
						cache_access (dtlb[threadid], Read, (addr & ~3), NULL, 4, sim_num_insn, NULL, NULL, NULL, 0);
#else
					cache_access (dtlb[threadid], Read, (addr & ~3), NULL, 4, sim_num_insn, NULL, NULL, 0);
#endif
				}
#endif
				...
				if (MD_OP_FLAGS (op) & F_CTRL)
				{
					...
					current->pred_PC = bpred_lookup (current->pred, current->regs.regs_PC, 1, op, MD_IS_CALL (op), MD_IS_RETURN (op), &dir_update, &stack_recover_idx, current->id, (bpred_btb_use_masterid ? current->masterid : current->id));

					if (!current->pred_PC)
						current->pred_PC = current->regs.regs_PC + sizeof (md_inst_t);

					bpred_update (current->pred,
							current->regs.regs_PC,
							current->regs.regs_NPC,
							current->regs.regs_NPC != (current->regs.regs_PC + sizeof (md_inst_t)),
							current->pred_PC != (current->regs.regs_PC + sizeof (md_inst_t)), current->pred_PC == current->regs.regs_NPC, op, &dir_update, current->id, (bpred_btb_use_masterid ? current->masterid : current->id));

					if (!(op == RETN || op == JMP || op == JSR || op == BSR || op == BR))
					{		/* Update the table direction table */
						if (current->regs.regs_NPC == current->regs.regs_PC + sizeof (md_inst_t))
							insert_br_dir (current->regs.regs_PC, 0, (current->pred_PC != current->regs.regs_NPC));	/* Not Taken branch */
						else
							insert_br_dir (current->regs.regs_PC, 1, (current->pred_PC != current->regs.regs_NPC));	/* Taken brnach */
					}
				}
#endif
#ifdef  STORE_PROFILE
				...
				{
					int memWidth = GetMemAccWidth (op);

					pushStore (current->regs.regs_PC, addr, current->sim_num_insn, memWidth);
				}
				...
				if ((MD_OP_FLAGS (op) & (F_MEM | F_LOAD)) == (F_MEM | F_LOAD))
				{
					...
					if (out1 > 0 && out1 < 63)
						verifyLoad (current->regs.regs_PC, addr, current->sim_num_insn, memWidth);
				}
#endif
#ifdef  INDIRECTJUMP_PROFILE
				if (((MD_OP_FLAGS (op) & (F_CTRL | F_UNCOND | F_INDIRJMP)) == (F_CTRL | F_UNCOND | F_INDIRJMP) && (op != RETN)))
					pushJump (current->regs.regs_PC, current->regs.regs_NPC);
#endif
#ifdef  FREQ_PROFILE
				instExecutedFreq (current->regs.regs_PC);
#endif

				current->regs.regs_PC = current->regs.regs_NPC;
				current->regs.regs_NPC += sizeof (md_inst_t);

				if ((sim_num_insn % 500000000) == 0)
				{
					printf ("[INFO]: sim_num_insn=%lld\n", sim_num_insn);
					fprintf (stderr, "[INFO]: sim_num_insn=%lld\n", sim_num_insn);
					fflush (stderr);
					fflush (stdout);
				}
			}
...
#else //DO_COMPLETE_FASTFWD
#ifdef	COLLECT_STAT_STARTUP
#ifdef	EDA
	if (numcontexts != 3)
		panic ("For execution in EDA mode there should be exactly 3 initial threads, each for correctness, optimistic and oracle core.");
#endif
...
#ifdef	EDA
	for (i = 0, j = 0; i < mta_maxthreads; i++)
	{
		if (j == THREADS_PER_JOB)
			break;

		if (thecontexts[i]->masterid == 0)
		{
			for (k = 0, l = 0; k < mta_maxthreads; k++)
			{
				if (thecontexts[k]->masterid == 1)
				{
					if (l == j)
					{
						int m;

						for (m = 0, l = 0; m < mta_maxthreads; m++)
						{
							if (thecontexts[m]->masterid == 2)
							{
								if (l == j)
								{
									thecontexts[i]->jobThdId = j;
									thecontexts[k]->jobThdId = j;
									thecontexts[m]->jobThdId = j;
									thecontexts[i]->helpThdId = k;
									thecontexts[k]->helpThdId = i;
									thecontexts[m]->helpThdId = -1;
									thecontexts[i]->oracleId = m;
									thecontexts[k]->oracleId = m;
									thecontexts[m]->oracleId = -1;

									thecontexts[i]->predQueue = (branchCorr *) calloc (1, sizeof (branchCorr));;
									initBranchCorr (thecontexts[i]->predQueue);
									thecontexts[k]->predQueue = thecontexts[i]->predQueue;
									thecontexts[m]->predQueue = thecontexts[i]->predQueue;
									break;
								}
								l++;
							}
	...
	for (;;)
	{
		...
		if (!spec_benchmarks && max_insts && !max_barrier)
		{
			for (threadid = 0; threadid < numcontexts; threadid++)
			{
				current = thecontexts[threadid];
#ifdef	EDA
				if (current->masterid == 0 && ((unsigned long) (current->sim_num_insn - WARMUP) >= (unsigned long) max_insts) && current->running)
#else
...
#ifdef	EDA
			if (((killall || tmp || (spec_benchmarks && max_insts && current->masterid == 0 && ((unsigned long) (current->sim_num_insn - WARMUP) >= (unsigned long) max_insts))) && current->running))
#else
...
#define PRINT_STATISTICS 0
		...
		for (i = 0; i < numcontexts; i++)
		{
			...
			{
				if ((thecontexts[i]->freeze == 1) && (thecontexts[i]->running == 0) && (thecontexts[i]->sleptAt + 100 + (random () % 20) < sim_cycle) && (!spec_benchmarks))
				{
					thecontexts[i]->freeze = 0;
					thecontexts[i]->running = 1;
					if (collect_stats)
...
#ifdef DIRECTORY
									lat = cache_access(cache_dl1[threadid], Write, (m_L1WBuf[threadid][i].addr&~3), NULL, 4, sim_cycle, NULL, NULL,/* RUU pointer */ NULL, threadid);  
#else
								lat = cache_access(cache_dl1[threadid], Write, (m_L1WBuf[threadid][i].addr&~3), NULL, 4, sim_cycle, NULL, NULL, threadid);  
#endif
							}
						}
#ifdef	DCACHE_MSHR
...
#ifdef	EDA
	void
assignFloatRegs (context * current, struct RUU_station *rs, int AorB, int dirty)
{
	enum md_opcode op;
	int x;
	md_inst_t inst;
	...
	if (AorB == 2)		/* Register C */
	{
		switch (op)
		{
			...
			case CVTQL:
				if (dirty)
					SET_FPR_Q (RC, rs->poison[2].data);
				else
					SET_FPR_Q (RC, rs->val_rc);
				break;
			default:
				if (dirty)
					SET_FPR (RC, rs->poison[2].data);
				else
					SET_FPR (RC, rs->val_rc);
				break;
		}
		return;
	}
	...
	switch (op)
	{
		...
		case FTOIS:
			...
			{
				if (AorB)
					SET_FPR_Q (x, rs->poison[1].data);
				else
					SET_FPR_Q (x, rs->poison[0].data);
			}
			else
			{
				if (AorB)
					SET_FPR_Q (x, rs->val_rb);
				else
					SET_FPR_Q (x, rs->val_ra);
			}
			break;
		case FCMOVEQ:
		case FCMOVGE:
...
/* Do call this routine for load or store instructions. */
	void
calculateWrongValue (struct RUU_station *rs)
{
	int i, j, k, x, y, z;
	int n_dispatched;		/* total insts dispatched */
	md_inst_t inst;		/* actual instruction bits */
	...
	inst = rs->IR;
	MD_SET_OPCODE (op, inst);
	current->regs.regs_PC = rs->PC;
	current->regs.regs_NPC = current->regs.regs_PC + sizeof (md_inst_t);

	if (!rs->poison[0].dirty)
	{
		if (rs->in1 < 32)
			SET_GPR (31 - rs->in1, rs->val_ra);
		else
			assignFloatRegs (current, rs, 0, 0);
	}
	else
	{
		if (rs->in1 < 32)
			SET_GPR (31 - rs->in1, rs->poison[0].data);
		else
			assignFloatRegs (current, rs, 0, 1);
	}
	if (!rs->poison[1].dirty)
	{
		if (rs->in2 < 32)
			SET_GPR (31 - rs->in2, rs->val_rb);
		else
			assignFloatRegs (current, rs, 1, 0);
	}
	else
	{
		if (rs->in2 < 32)
			SET_GPR (31 - rs->in2, rs->poison[1].data);
		else
			assignFloatRegs (current, rs, 1, 1);
	}
...
#include "machine.def"
	...
	if (MD_OP_FLAGS (rs->op) & F_CTRL && !rs->spec_mode && !rs->isWrongPath)
	{
		rs->changeSpecMode = 0;
		if (rs->altPC != rs->next_PC)
		{
			if ((current->regs.regs_NPC == rs->altPC) && (rs->pred_PC == rs->next_PC))
			{
				rs->changeSpecMode = 1;
				//badBranchFullCorrect++;
			}
			else if ((current->regs.regs_NPC == rs->altPC) && (rs->pred_PC != rs->next_PC))
			{
				rs->changeSpecMode = 2;
				//badBranchSpecCorrect++;
			}
		}
		else
		{
			...
			{
				rs->changeSpecMode = 3;
				rs->altPC = current->regs.regs_NPC;
				//badBranchFullRec++;
			}
			else if ((current->regs.regs_NPC != rs->altPC) && (rs->pred_PC != rs->next_PC))
			{
	...
	else
	{
		...
		{
			rs->poison[2].dirty = 1;
			rs->poison[2].data = addr;
		}
		else if (MD_OP_FLAGS (rs->op) & F_STORE)
		{			/* For stores, dirty is already marked */
		}
		else if (val_rc != rs->val_rc)
		{
			rs->poison[2].dirty = 1;
			rs->poison[2].data = val_rc;
		}
		else
		{
			rs->poison[2].dirty = 0;

 ...
 *
 * $Id: stats.c,v 1.1.1.1 2011/03/29 01:04:34 huq Exp $
 *
 * $Log: stats.c,v $
	   ...
	   FILE *fd)			/* output stream */
{
  ...
  for (i=0; i<stat->variant.for_dist.arr_sz; i++)
    {
      ...
      bavg = btotal / max2((double)bcount, 1.0);
  ...
  fprintf(fd, "%s.average = %8.4f\n", stat->name, btotal/max2(bcount, 1.0));
	  ...
	  if (stat->variant.for_dist.print_fn)
	      ...
	      if (stat->format == NULL)
		{
		  ...
		  if (pf & PF_PDF)
		    fprintf(fd, "%6.2f ",
			    (double)stat->variant.for_dist.arr[i] /
			    max2(btotal, 1.0) * 100.0);
		  if (pf & PF_CDF)
		    fprintf(fd, "%6.2f ", bsum/max2(btotal, 1.0) * 100.0);
		}
	      else
		{
	    ...
	    FILE *fd)			/* output stream */
{
  ...
  if (bcount > 0)
      ...
      for (bindex=0,i=0; i<HTAB_SZ; i++)
	{
      ...
      qsort(barr, bcount, sizeof(struct bucket_t *), (qsort_func) compare_fn);
      ...
      for (i=0; i<bcount; i++)
	{
	  ...
	  if (stat->variant.for_sdist.print_fn)
	      ...
	      if (stat->format == NULL)
		{
		  ...
		  if (pf & PF_PDF)
		    fprintf(fd, "%6.2f ",
			    (double)barr[i]->count/max2(btotal, 1.0) * 100.0);
		  if (pf & PF_CDF)
		    fprintf(fd, "%6.2f ", bsum/max2(btotal, 1.0) * 100.0);
		}
	      else
	...
	if (eval_error != ERR_NOERR || *endp != '\0')
	  {
	    if (eval_err_extra_info)
	      fprintf(fd, "<error: %s: %s>", eval_err_str[eval_error], eval_err_extra_info);
	    else
